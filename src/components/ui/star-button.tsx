...
export function StarButton({
  ...
  const sharedClass = cn(
    "relative z-[3] overflow-hidden h-10 px-4 py-2 inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-3xl text-sm font-medium transition-colors disabled:pointer-events-none disabled:opacity-50 group/star-button cursor-pointer",
    className,
  );

  if (href) {
    return (
      <a href={href} style={sharedStyle} className={sharedClass}>
        {inner}
      </a>
    );
  }
